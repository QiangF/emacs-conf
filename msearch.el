;; 第一步: 找到开启高亮的代码入口
;; 第二步: 任何时候有文本被选中时, 进入该入口, 不限于鼠标选择
;; 鼠标释放的时候才会高亮, 应该鼠标移动的过程中动态的这样做. 
  ...
  (let ((handler-key (kbd (concat "<" e ">")))
    ...
    `(progn
      ...
      (defun ,handler (e)
        ...
        (let ((n ,handler-list))
          (while n
            ;(message (concat "nth loop in handler-list" (format "%s" n)))
            (apply (car n) (list e))
            (setq n (cdr n)))))
...
(defun msearch-event-handler (e)
  ...
  (interactive "e")
  ;;(print "when this is executed")
  (let (start end (modifiers (event-modifiers e)))
    ;;(message "whats the modifiers")
    ;;(print modifiers)
    (cond
